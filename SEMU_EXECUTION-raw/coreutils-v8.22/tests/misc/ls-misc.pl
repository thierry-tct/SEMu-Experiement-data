...
# MFI Split test
if (exists($ENV{'MFISPLITTESTS_SPLIT_ID'}))
{
    #print "MFI_SPLIT_NUM ".@Tests."\n"; die;
    my $mfi_split_id = int($ENV{'MFISPLITTESTS_SPLIT_ID'});
    @Tests = ($Tests[$mfi_split_id]);
}
...
setuid_setup;
my $fail1 = run_tests ($ME, $prog, \@Tests, $save_temps, $verbose);
#$fail
#  and exit 1;
...
setuid_setup;
my $fail2 = run_tests ($ME, $prog, \@Tests, $save_temps, $verbose);
$fail1 
  and exit 1;
exit $fail2;
